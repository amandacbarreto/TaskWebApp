...
import com.webapp.WebApp.model.Task;
...
public class TaskController {
    ...
    public String create (Task task){
        if(task.getId()!=null){
            Long id = task.getId();
            Task taskObj = tasks.stream().filter(taskItem -> id.equals(taskItem.getId())).findFirst().get();
            tasks.set(tasks.indexOf(taskObj), task);
        } else {
